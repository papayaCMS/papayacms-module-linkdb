...
* @version $Id: content_linkdb.php 39512 2014-03-04 15:57:18Z weinert $
*/
...
class content_linkdb extends base_content {
  ...
  */
   ...
   * @var output_linkdb
   */
  private $linkDatabaseObject = NULL;
  ...
  */
  function getLinkDbCombo($name, $field, $data) {
    ...
    if (isset($this->linkDatabaseObject->categs) &&
      ...
      $result .= $this->getCategSubTree(0, 0, (int)$data);
    }
    $result .= '</select>'.LF;
    return $result;
  }
  ...
  */
  function getCategSubTree($parent, $indent, $selectedId) {
    $result = '';
    if (isset($this->linkDatabaseObject->categTree[$parent]) &&
      ...
      foreach ($this->linkDatabaseObject->categTree[$parent] as $id) {
        $result .= $this->getChildCategs($id, $indent, $selectedId);
      }
    }
    return $result;
  }
   ...
   */
  function getChildCategs($id, $indent, $selectedId) {
    $result = '';
    if (isset($this->linkDatabaseObject->categs[$id]) &&
      ...
      $selected = ($selectedId == $this->linkDatabaseObject->categs[$id]['linkcateg_id'])
        ? ' selected="selected"' : '';
      $result .= sprintf(
        '<option value="%d"%s>%s</option>'.LF,
      ...
      $result .= $this->getCategSubTree($id, $indent + 1, (int)$selectedId);
    }
    return $result;
  }
}
