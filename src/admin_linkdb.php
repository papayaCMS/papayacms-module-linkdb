...
* @version $Id: admin_linkdb.php 39818 2014-05-13 13:15:13Z weinert $
*/
...
class admin_linkdb extends base_linkdb {
  ...
  */
   ...
   * @var base_dialog
   */
  private $linkDialog = NULL;

  /**
   * @var array
   */
  protected $localImages = array();
  ...
  function getXMLCategTree() {
    if (isset($this->categs) && is_array($this->categs)) {
      ...
      $result .= sprintf(
        ...
        $this->buildLink(array('categ_id' => 0)).'&ldb[link_id]=0',
        $this->_gt('Base'),
        $images['places-desktop'],
        $selected
      );
  ...
  */
  function getPagesNav(
    $toolbar, $offset, $step, $max, $paramName = 'offset', $caption = 'Pages'
  ) {
  ...
  function getXMLCutedElements () {
    ...
    if (isset($this->cuttedLink) && is_array($this->cuttedLink)) {
      foreach ($this->cuttedLink as $cuttedLink) {
        ...
        case 0:
          $image = $images['status-sign-warning'];
          break;
        case 1:
          $image = $images['status-sign-ok'];
          break;
        case 2:
          $image = $images['status-sign-off'];
          break;
        default :
          $image = '';
          break;
  ...
  */
  function getXMLResetCounterByLinkForm() {
    ...
    $msg = sprintf($this->_gt('Reset counter').' ('.$this->link['link_title'].')?');
    $dialog = new base_msgdialog($this, $this->paramName, $hidden, $msg, 'question');
    $dialog->baseLink = $this->baseLink;
    $dialog->buttonTitle = 'Reset';
    $this->layout->add($dialog->getMsgDialog());
  ...
  */
  function cutLink($linkId) {
    ...
    return FALSE !== $this->databaseUpdateRecord(
      $this->tableLinkdb, $data, 'link_id', (int)$linkId
    );
  }
  ...
  */
  function cutCateg($categId) {
    ...
    return FALSE !== $this->databaseUpdateRecord(
      $this->tableLinkdbCateg, $data, 'linkcateg_id', (int)$categId
    );
  }
  ...
  * @return mixed FALSE or number of affected_rows or database result object
  */
  function pasteCateg($parent, $categId) {
    $this->loadCateg($this->params['categ_id']);
    if (isset($this->categ) && is_array($this->categ)) {
      $path = $this->categ['linkcateg_path'].$this->categ['linkcateg_id'].';';
    } else {
